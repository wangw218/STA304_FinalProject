...
abstract: |
  This paper investigated whether Justin Trudeau or Andrew Scheer would win 2019 Canadian Federal Election if "everyone" had voted by using MRP technique based on GSS and CES datasets. The result is that Justin Trudeau would still win even if everyone had voted. The analysis on the topic is important since it might increase the participtions of citizens during the next election period, especially if the election reulst would change, and this might result in a tremendous change in the future of a country.
  **keywords**: 2019 Canadian Federal Election; MRP; everyone voted;
...
I did some data cleaning to "match the variables names and values between the two source files and produced the cleaned data file: survey_data.csv and census_data.csv. (More explanation of how to get these two files are explained in README.md)
...
"province" with range ("Quebec","Manitoba","Ontario","Alberta","Nova Scotia","British Columbia", "Saskatchewan", "Prince Edward Island", "New Brunswick", "Newfoundland and Labrador") for census data and three more territories: Yukon, Northwest Territories and Nunavut, for survey data. The distribution of the province aboout survey and census data can be found in Appdendix A.
...
To do Post-Stratification on Liberal Party's model: the post-stratification formula (Equation \@ref(eq:eq5)) would be used to calculate the probability of voting for the Liberal Party for each province:
...
Post-Stratification on the Conservative Party's model would use a same equation as the Liberal Party's except $\hat{y}^{PS}$ is now the estimated probability of voting for theConservative Party and  $\hat{y}_{j}$ is the estimate of probability of voting for the Conservative Party in each cell for a particular province.
...
## Model Check
Before moving to the result, I discuss if the models I chose are good and valid. I will verify this by using the ROC curve technique. If the area under curve (AUC), is greater than 0.5, I could consider my models are good [@AUC].
...
From the Figure \@ref(fig:ROCLib) and Figure \@ref(fig:ROCCon), we could see the AUC of both of them is much greater that 0.5. Therefore, I could conclude my models are good and valid.
...
According to Figure \@ref(fig:seatsResult), I estimate that the number of seats theLiberal Party got would be 104 and the number of seats the Conservative Party got would be 97. This is based off our post-stratification analysis of the proportion of voters in each province in favour of the Liberal Party and the Conservative Party modeled by two multilevel regression models with a binomial family, where the level 1 logistic model accounted for sex, age_group, household_income and level 2 model accounted for province.
...
## 4. Discussion
In the paper, I invested if the 2019 Canadian federal election result would change if "everyone" had voted, specifically, if Andrew Scheer would win the election instead of Justin Trudeau, by using MRP technique. I chose the 2019 Canadian Election Study - Online Survey as my survey data and the General social survey on Family (cycle 31), 2017 as my census data. First, I cleaned the data in order to do further analysis. I chose four common and meaningful variables between the survey and census data: sex, household income, age, province to do the analysis. In order to do post-stratification, I classified the continuous variables, household income and age, to be categorical with the same range. In addition, I made two new binary variables, voteLib and voteCon, based on the variable "cps19_votechoice" with the question being "Which party do you think you will vote for?" in Survey data. voteLib stands for "voting for Liberal Party"and value 1 means the respondent would like to vote for it and 0 means not. voteCon has the same logic but it's for Conservative Party. The overview of the cleaned data of this report can be found in Table \@ref(tab:survey)and Table \@ref(tab:census). The cleaned survey data has 21513 samples while cleaned census data size is only 20178. Obviously, this is a drawback.

After the data were cleaned, I built two multilevel Regression models using a binomial family based on the survey data to predict the probability of voting Liberal Party and Conservative Party. Then I use the two models to do post-stratification based on province on census data, splitting cells based on sex, household income and age group. After Post-Stratification, we would get the estimated probability of voting for these two Parties among the 10 provinces, as shown on Table \@ref(tab:resultT) and Table \@ref(tab:resultS). Then I estimate the number of seats each party got from every province by the product of the probability of the party got voted in that province and the number of seats that province have according to the data on Table \@ref(tab:seatTable). Then the total number of seats each party got is the sum of the seats of all provinces. Finally, I estimate Liberal Party would get 104 seats and Conservative Party would get 97 seats (Figure \@ref(fig:seatsResult)), so Justin Trudeau would still win the 2019 Canadian federal election even if "everyone" had voted.

In our analysis, Justin Trudeau would still win the 2019 Canadian federal election even if "everyone" had voted but the Liberal Party only have 7 more seats than Conservative Party. However, in other cases, the governing party might change and this might have a great influence on the country and the world. Like the example in the Introduction section, if "everyone" had voted in 2016 US presidential election, Hillary Clinton would probably be president [@CiteIntro1] and the situation about COVID-19 in America might not be such terrible, 17,515,091 confirmed cases with 313,748 deaths up to 21st December, 2020 according to World Health Organization [@citeWHO]. This would encourage every eligible citizen in each country to vote during the election period.

### 4.2 Weaknesses
Generally, there are four weaknesses in our model and prediction.

First, the cleaned survey dataset has 21513 samples while the cleaned census data size is only 20178, which means the cleaned census data size is too small, which would increase the bias.

Second, the census data are collected excluding those from Yukon, Northwest Territories, and Nunavut, which would increase the bias of our models.

Third, this one is based on the second one. Since the census data excludes people from Yukon, Northwest Territories, and Nunavut. When we do post-stratification on the census data, we could not get the estimates of the probabilities of voting Liberal Party and Conservative Party among these three territories, which would result in the lower estimates of total seats each party got, although these three territories only have 3 seats and would not change our final prediction result in this case since the Liberal Party have 7 more seats than Conservative Party.

Fourth, our models only use 4 variables: sex, age group, household income and province. This is not a large number of explanatory variables. If more variables like education and race could be used in our models, the prediction result would be more accurate and less biased.

### 4.3 Next Steps
There are 3 next steps we could do to improve our analysis.

First, find a greater census dataset that contains all regions of Canada to build models.

Second, include more explanatory variables in our models to make the model better.
...
Figure \@ref(fig:surveyP) and Figure \@ref(fig:censusP) show the total observations of different provinces in survey Data and census data respectively, where we could see in the survey data there are three more territories: Yukon, Northwest Territories and Nunavut and the respondents in these three territories are much less than those in the other 10 provinces.
...
From the Table \@ref(tab:Stable), most variables about age_group, sex and household_income are significant since their p values are small. This multilevel regression model is a good model for predicating the probability of voting for the Conservative Party.
